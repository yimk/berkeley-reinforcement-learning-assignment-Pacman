...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(0, iterations):
            ...
            for current_state in states:
                values = []
                for action in mdp.getPossibleActions(current_state):
                    values.append(self.computeQValueFromValues(current_state, action))

                if not values:
                    tmpValues[current_state] = 0
                else:
                    tmpValues[current_state] = max(values)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for new_state, prob in transition_functions:
